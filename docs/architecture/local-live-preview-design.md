...
初回mirrorにはarticle bundle内のpage resourceも含まれる。workspace作成後にCMS media APIでcontent directory配下へupload/deleteしたresourceは、active shadow workspaceへ同じ変更を同期する。Eleventyでは同期後にmetadata invalidationを行い、削除resourceのwatch event取りこぼし時は対象pathの親directoryを再通知してwatch rebuildを回復させる。static resourceはproduction-linked treeを監視するためshadowへコピーせず、同じinvalidation契約だけを適用する。
