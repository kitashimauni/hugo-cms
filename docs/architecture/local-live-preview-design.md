...
初回mirrorにはarticle bundle内のpage resourceも含まれる。workspace作成後にCMS media APIでcontent directory配下へupload/deleteしたresourceは、production mutation前にmetadataをinvalidateし、active shadow workspaceへ同じ変更を同期した後にも再度invalidateする。これによりresource変更中は旧metadataを解決できず、最終変更後のwatch rebuildとURL map更新を待てる。削除resourceのwatch event取りこぼし時は対象pathの親directoryを再通知してwatch rebuildを回復させる。static resourceはproduction-linked treeを監視するためshadowへコピーせず、同じinvalidation契約だけを適用する。

static配下はHugoが元repositoryを直接参照するためshadow同期しない。static resource変更がURL mapに影響しない場合でも、Eleventyのproduction-linked treeではmetadata invalidation後のwatch rebuildを許可する。media本体の保存が成功した後にpreview-only同期またはinvalidationが失敗した場合、media APIを失敗扱いにはせずserver logへ残す。
