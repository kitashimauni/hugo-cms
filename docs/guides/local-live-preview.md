...
`POST /admin/api/sync`がproduction repositoryのpullに成功した場合、対象siteのLocal Live Previewも同じcleanup gateでresetします。generator processを停止し、Eleventyのproject overlayを含むshadow workspaceをdetachして非同期cleanupするため、contentだけでなくlayouts、config、static、package.json、lockfileなどの変更も次回Preview起動時に反映されます。Sync中にfrontendから送信済みだったLocal Preview updateは、Sync開始前に完了を待って停止し、古いpayloadでreset後のworkspaceを復活させないようにします。

Git Sync自体が成功してもPreview resetに失敗する場合があります。そのときAPIは`local_preview_reset: false`を返し、productionのSync成功とPreviewの失敗を分離して表示します。frontendはPreview URLとiframeをresetするため、server logを確認したうえで次回Preview表示時に再起動できます。Preview resetはproduction working treeやGit stateを変更しません。Sync時にeditorがcleanなら現在の記事をproduction treeから再ロードし、remoteで削除されていた記事は選択状態も解除します。未保存editor内容は自動破棄せず保持するため、Git Sync後にPreviewを開くとその現在payloadがshadowへ同期されます。remoteで削除された記事に未保存内容がある場合も、復旧できるようeditorは保持します。
